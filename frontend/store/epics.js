  ...
  action$.pipe(
    ...
    mergeMap((action) => {
      return interval(5000).pipe(
        ...
        takeUntil(action$.ofType(types.STOP_FETCHING_USERS, types.FETCH_USER_FAILURE))
      );
    })
  );
  ...
  action$.pipe(
    ...
    mergeMap((action) =>
      request({
        ...
        map((response) => actions.fetchUserSuccess(response.response, action.payload.isServer)),
        catchError((error) =>
          of(actions.fetchUserFailure(error.xhr.response, action.payload.isServer))
        )
      )
    )
  );
